...
class Boule:
    def __init__(self):
        ...
        self.y = HAUTEUR - 100
        self.rayon = 25
        self.deplacement = 0  # Seulement pour BCI

    def update_bci(self, commande):
        """commande = 'gauche', 'droite' ou None"""
        if commande == "gauche":
            self.x -= 30
    ...
    def update_clavier(self, keys):  # Fallback clavier
    ...
    def draw(self, screen):
        pygame.draw.circle(screen, BLEU, (int(self.x), int(self.y)), self.rayon)
...
class JeuBCI:
    ...
    def update(self):
        ...
        self.spawn_timer += 1
        if self.spawn_timer > 120:  # ~2s à 60fps
    ...
    def run(self):
        ...
        while running:
            ...
            # SIMULATION BCI (remplacer par vraie interface)
            # Remplace cette partie par ta réception série/BCI
            keys = pygame.key.get_pressed()
            if keys[K_SPACE]:  # Espace = commande simulée
                commande = random.choice(["gauche", "droite", None])
