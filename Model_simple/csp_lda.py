...
def load_edf(edf_path: str):
    ...
    Applique :
      ...
      - Rejet d artefacts (z-score > 3.5 sur le peak-to-peak)
    ...
    event_id = {ann: i + 1 for i, ann in enumerate(LABEL_MAP)}
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
    ...
    y        = np.array([LABEL_MAP[inv[e]] for e in epochs.events[:, 2]])
...
if __name__ == "__main__":
    ...
    for edf_path in edfs:
        ...
        print(f"\n  Chargement {name}.edf ...")
        X, y, ch_names, _ = load_edf(edf_path)
        print(f"  X : {X.shape}   ({(y=='left_hand').sum()} G"
              f" / {(y=='right_hand').sum()} D)")
        print(f"  Canaux : {ch_names}")

        model = CSPLDA(n_components=N_COMP, n_folds=N_FOLDS)
        cv_result = model.cv_evaluate(X, y)
        all_results[name] = cv_result
        print_report(name, cv_result)

        # Entraine sur TOUTES les donnees et sauvegarde
        model.fit(X, y)
